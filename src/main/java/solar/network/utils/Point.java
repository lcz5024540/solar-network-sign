...
public class Point {
    ...
    public static Point add(Point P1, Point P2) {
        ...
        BigInteger lam = null;
        if(P1.equals(P2)) {
            BigInteger base = P2.getY().multiply(new BigInteger("2"));
            lam = (BigInteger.valueOf(3L).multiply(P1.getX()).multiply(P1.getX()).multiply(base.modPow(p.subtract(new BigInteger("2")), p))).mod(p);
        }
        else {
            BigInteger base = P2.getX().subtract(P1.getX());
            lam = ((P2.getY().subtract(P1.getY())).multiply(base.modPow(p.subtract(new BigInteger("2")), p))).mod(p);
        }
    ...
    public static boolean isSquare(BigInteger x) {
        return x.modPow(p.subtract(BigInteger.ONE).divide(new BigInteger("2")), p).compareTo(BigInteger.ONE) == 0;
    }
    ...
    public static byte[] taggedHash(String tag, byte[] msg) throws NoSuchAlgorithmException {
        ...
        byte[] tagHash = HexUtil.sha256(tag.getBytes());
        ...
        return HexUtil.sha256(buf);
    }

    public static byte[] genPubKey(byte[] secKey) throws Exception {
        BigInteger x = HexUtil.bigIntFromBytes(secKey);
    ...
    public static byte[] bytesFromPoint(Point P) {
        return HexUtil.bytesFromBigInteger(P.getX());
    }

    public static Point pointFromBytes(byte[] b) {

        BigInteger x = HexUtil.bigIntFromBytes(b);
        if(x.compareTo(p) >= 0) {
            return null;
        }
        ...
        if(y.modPow(new BigInteger("2"), p).compareTo(y_sq) != 0) {
            return null;
        }
        else {
